...
class IARecommandationService
{
    ...
    private function analyserLocalement(string $symptomes, ?string $antecedents, string $listeProduits): array
    {
        ...
        $motsClés = [
            ...
            'gorge' => ['gorge', 'angine', 'pharyngite', 'mal a la gorge', 'avaler'],
            'respiration' => ['respiration', 'poumon', 'bronche', 'asthme', 'souffle', 'respirer'],
            'fievre' => ['fievre', 'temperature', 'chaud', 'chaleur', 'transpiration'],
            'douleur' => ['douleur', 'mal', 'souffrance', 'douloureux', 'fait mal'],
        ];
        ...
        // Extraire les IDs des produits de la liste
        preg_match_all('/ID: (\d+) \| Nom: ([^\|]+) \| Catégorie: ([^\|]+)/', $listeProduits, $matches, PREG_SET_ORDER);
