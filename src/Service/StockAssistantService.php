...
class StockAssistantService
{
    ...
    public function analyserQuestion(string $question): array
    {
        ...
        try {
            ...
            return [
                'intent' => 'error',
                'analysis' => 'Erreur: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine(),
                'prevision' => ['items' => [], 'count' => 0],
    ...
    private function analyseRuptureSoon(string $question, array $stocks, int $daysWindow): array
    {
        ...
        foreach ($stocks as $stock) {
            ...
            $items[] = [
                'produit' => $stock->getProduit()?->getNom() ?? 'Non assigné',
                'depot' => $stock->getDepot()?->getNomDepot() ?? 'Non assigné',
                'quantite' => $quantite,
    ...
    private function analyseDepotPerformance(string $question, array $stocks): array
    {
        ...
        foreach ($stocks as $stock) {
            ...
            $stats[$id] ??= [
                'depot' => $depot->getNomDepot() ?? 'Non assigné',
                'total_stocks' => 0,
                'total_quantite' => 0,
                'total_sorties' => 0,
                'total_entrees' => 0,
