...
class StockService
{
    ...
    public function transfererStock(Stock $stockSource, Stock $stockDestination, int $quantite, string $motif = null): bool
    {
        ...
        $depotDestination = $stockDestination->getDepot();
        $depotSource = $stockSource->getDepot();
        $nomDepotDestination = $depotDestination?->getNomDepot() ?? 'Dépôt inconnu';
        $nomDepotSource = $depotSource?->getNomDepot() ?? 'Dépôt inconnu';
        
        $success = $this->sortirStock($stockSource, $quantite, "Transfert vers {$nomDepotDestination}: {$motif}");
        
        if ($success) {
            // Entrée dans le stock destination
            $this->entrerStock($stockDestination, $quantite, "Transfert depuis {$nomDepotSource}: {$motif}");
        }
