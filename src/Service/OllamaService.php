...
class OllamaService
{
    ...
    public function __construct(
        ...
        // Use default if env var is empty, and ensure 127.0.0.1 for Windows
        $url = !empty($ollamaUrl) ? $ollamaUrl : 'http://127.0.0.1:11434';
        $this->baseUrl = str_replace('localhost', '127.0.0.1', $url);
        $this->model = !empty($model) ? $model : 'llama3';
        $this->logger = $logger;
    }
    ...
    public function isAvailable(): bool
    {
        try {
            ...
            if ($this->logger) {
                $this->logger->warning('Ollama not available at ' . $this->baseUrl . ': ' . $e->getMessage());
            }
            return false;
        }
    }
    ...
    /**
     * Generate a response using Ollama with an optional system prompt and timeout
     */
    public function generateResponse(string $message, array $conversationHistory = [], string $systemPrompt = null, int $timeout = 120): string
    {
        try {
            // Build context from conversation history or use provided system prompt
            $context = $systemPrompt ?? $this->buildContext($conversationHistory);
            ...
            throw $e; // Throw exception so controller can handle it
        }
    }
