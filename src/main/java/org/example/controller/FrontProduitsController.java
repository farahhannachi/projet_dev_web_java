...
public class FrontProduitsController {
    ...
    private void renderProductCards(List<Produit> source) {
        ...
        for (Produit p : source) {
            ...
            double originalPrice = p.getPrix();
            double promoPrice = promotionService.getPromotionalPrice(p.getId(), originalPrice);
            boolean hasPromo = promoPrice < originalPrice - 0.001;

            VBox priceBox = new VBox(2);
            if (hasPromo) {
                // Prix barré original
                Label originalLabel = new Label(String.format("%.2f DT", originalPrice));
                originalLabel.getStyleClass().add("front-product-price-original");
                originalLabel.setStyle("-fx-text-fill: #9ca3af; -fx-font-size: 12px; -fx-strikethrough: true;");

                // Prix promo en vert
                Label promoLabel = new Label(String.format("%.2f DT", promoPrice));
                promoLabel.getStyleClass().add("front-product-price");
                promoLabel.setStyle("-fx-text-fill: #16a34a; -fx-font-weight: bold; -fx-font-size: 15px;");

                // Badge réduction
                double pct = Math.round((1 - promoPrice / originalPrice) * 100);
                Label badge = new Label("-" + (int) pct + "%");
                badge.setStyle("-fx-background-color: #dc2626; -fx-text-fill: white; -fx-font-size: 11px; "
                        + "-fx-font-weight: bold; -fx-padding: 2 6 2 6; -fx-background-radius: 4;");

                HBox promoRow = new HBox(6, promoLabel, badge);
                promoRow.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
                priceBox.getChildren().addAll(originalLabel, promoRow);
            } else {
                Label priceLabel = new Label(String.format("%.2f DT", originalPrice));
                priceLabel.getStyleClass().add("front-product-price");
            ...
            card.getChildren().addAll(imageView, name, desc, priceBox, stock, addBtn);
