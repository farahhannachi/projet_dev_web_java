...
/**
 * Page « À propos » : contenu marketing HTML/CSS dans un {@link WebView}, navbar alignée sur les autres écrans client.
 */
public class AProposController {
    ...
    public void initialize() {
        ...
        if (webView != null) {
            ...
            try {
                String css = readUtf8("/html/about-page.css");
                String body = readUtf8("/html/apropos-body.html");
                ...
                engine.loadContent("<html><body style='font-family:sans-serif;padding:2rem;'>"
                        + "<p>Impossible de charger la page À propos.</p><pre>"
    ...
    private static String escapeForStyle(String css) {
        return css.replace("</", "<\\/");
    }
    ...
    private static String readUtf8(String classpathPath) throws IOException {
        try (InputStream in = AProposController.class.getResourceAsStream(classpathPath)) {
            if (in == null) {
