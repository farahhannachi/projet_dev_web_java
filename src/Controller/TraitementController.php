...
class TraitementController extends AbstractController
{
    ...
    public function index(Request $request, TraitementRepository $traitementRepository, PaginatorInterface $paginator): Response
    {
        ...
        $sortByValue = $request->query->get('sortBy', 'id_desc');
        ...
        // Filtre par statut
        if ($filterStatut) {
            // Rendre la recherche plus robuste pour gérer les accents, les espaces et les underscores
            $queryBuilder->andWhere('t.status = :statut OR t.status = :statut_space OR t.status = :statut_underscore')
                ->setParameter('statut', $filterStatut)
                ->setParameter('statut_space', str_replace('_', ' ', $filterStatut))
                ->setParameter('statut_underscore', str_replace(' ', '_', $filterStatut));
        }
    ...
    public function search(Request $request, TraitementRepository $traitementRepository, PaginatorInterface $paginator): Response
    {
        $this->denyAccessUnlessGranted('ROLE_ADMIN');
        ...
        try {
            ...
            $html = $this->renderView('Admin/traitements/_search_results.html.twig', [
                'traitements' => $pagination
            ]);
            ...
            return $this->json([
                'success' => false,
                'message' => 'Erreur lors du rendu des résultats: ' . $e->getMessage(),
