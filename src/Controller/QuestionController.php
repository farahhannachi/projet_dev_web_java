...
class QuestionController extends AbstractController
{
    ...
    public function index(Request $request, QuestionRepository $questionRepository, ResponseQuestionRepository $responseRepository): Response
    {
        ...
        $priorite = (string) $request->query->get('priorite', '');
        ...
        if ($this->isGranted('ROLE_ADMIN')) {
            ...
            $questions = $questionRepository->findWithFilters($statut ?: null, $priorite ?: null, $type ?: null, $search ?: null, $sort, $order);
            $template = 'Admin/tickets.html.twig';
            $unreadByTicket = [];
        } else {
            // Un client voit seulement ses questions
            $userId = $user ? $user->getId() : 0;
            $questions = $questionRepository->findByUtilisateur($userId);
...
): Response
{
    ...
    $question->setUtilisateur($currentUser instanceof Utilisateur ? $currentUser : null);
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        if ($fichier) {
            ...
            try {
                ...
                $uploadDir = $projectDir.'/public/uploads/questions';
                if (!is_dir($uploadDir)) {
    ...
    public function show(
        ...
        $reponses = $questionId !== null ? $responseRepository->findByQuestion($questionId) : [];
        ...
        $responseQuestion->setUtilisateur($currentUser instanceof Utilisateur ? $currentUser : null);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($fichier) {
                ...
                try {
                    $projectDirParam = $this->getParameter('kernel.project_dir');
                    $projectDir = is_string($projectDirParam) ? $projectDirParam : '';
                    $uploadDir = $projectDir.'/public/uploads/responses';
                    if (!is_dir($uploadDir)) {
                        mkdir($uploadDir, 0777, true);
                    }
    ...
    public function updateStatut(Question $question, Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        $nouvellePriorite = (string) $nouvellePriorite;
