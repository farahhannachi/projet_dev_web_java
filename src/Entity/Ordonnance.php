...
class Ordonnance
{
    ...
    #[ORM\Column(name: 'date_ordonnance', type: 'datetime_immutable')]
    #[Assert\NotBlank(message: 'La date de l\'ordonnance est obligatoire')]
    private \DateTimeImmutable $dateOrdonnance;
    ...
    #[ORM\Column(name: 'signature_date', type: 'datetime_immutable', nullable: true)]
    ...
    #[ORM\Column(name: 'signature_patient_date', type: 'datetime_immutable', nullable: true)]
    private ?\DateTimeImmutable $signaturePatientDate = null;

    #[ORM\Column(name: 'signature_patient_ip', length: 45, nullable: true)]
    private ?string $signaturePatientIp = null;

    #[ORM\ManyToOne(targetEntity: Utilisateur::class)]
    #[ORM\JoinColumn(name: 'id_utilisateur_id', referencedColumnName: 'id_utilisateur', nullable: true, onDelete: 'CASCADE')]
    #[Assert\NotBlank(message: 'Le patient est obligatoire')]
    private ?Utilisateur $utilisateur = null;

    #[ORM\OneToMany(targetEntity: Traitement::class, mappedBy: 'ordonnance', cascade: ['persist', 'remove'], orphanRemoval: true)]
    ...
    protected function setDateOrdonnance(\DateTimeImmutable $dateOrdonnance): static
    {
    ...
    /**
     * Get noteMedical with encoding fixed
     * Fixes multiple layers of UTF-8 encoding issues (mojibake)
     */
    public function getNoteMedicalFixed(): ?string
    {
        if (empty($this->noteMedical)) {
            return $this->noteMedical;
        }

        $text = $this->noteMedical;
        
        // Try multiple encoding fixes iteratively
        for ($i = 0; $i < 5; $i++) {
            $before = $text;
            
            // Check if still contains mojibake
            if (strpos($text, 'Ã') === false && strpos($text, 'Â') === false) {
                break;
            }
            
            // Try converting from ISO-8859-1 to UTF-8
            ...
            // Try converting from Windows-1252 to UTF-8
        ...
        // Additional fix for remaining mojibake with direct replacement
        if (strpos($text, 'Ã') !== false || strpos($text, 'Â') !== false) {
            // Direct string replacements for common garbled characters
    ...
    protected function setSignatureDate(?\DateTimeImmutable $signatureDate): static
    {
    ...
    public function getSignaturePatientDate(): ?\DateTimeImmutable
    {
        return $this->signaturePatientDate;
    }

    protected function setSignaturePatientDate(?\DateTimeImmutable $signaturePatientDate): static
    {
